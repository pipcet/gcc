...
const char *
wasm32_jsexport_format_type (tree type)
{
...
void
wasm32_jsexport_declare_type (tree type, struct wasm32_jsexport_opts *opts,
			     vec<struct wasm32_jsexport_decl> *decls)
{
  ...
  if (TREE_CODE (type) == RECORD_TYPE) {
    ...
    ret.fragments.safe_push (concat (wasm32_jsexport_format_type (type),
				     ";", NULL));
    decls->safe_push (ret);
  }
}
...
tree
wasm32_canonicalize_type (tree type)
{
  if (TREE_CODE (type) == POINTER_TYPE)
    return build_pointer_type (wasm32_canonicalize_type (TREE_TYPE (type)));
...
void
wasm32_jsexport_type (tree type, struct wasm32_jsexport_opts *opts,
		     vec<struct wasm32_jsexport_decl> *decls);
			     ...
			     vec<struct wasm32_jsexport_decl> *decls)
{
  ...
  const char *typestr = wasm32_jsexport_format_type (build_pointer_type (type));
  ...
  wasm32_jsexport_declare_type (type, opts, decls);
  ret.fragments.safe_push(concat ("wasm32_register_record_type(\"", ret.jsname,
			     "\", ", typestr, ");", NULL));
  ...
  for (field = TYPE_FIELDS (type); field && TREE_CODE (field) == FIELD_DECL; field = TREE_CHAIN (field))
	...
	fieldtype = TYPE_CANONICAL (fieldtype);
      const char *fieldtypestr = wasm32_jsexport_format_type (build_pointer_type (fieldtype));
	...
	continue;
      ...
      wasm32_jsexport_declare_type (fieldtype, opts, decls);
      wasm32_jsexport_type (fieldtype, opts, decls);

      ret.fragments.safe_push(concat ("wasm32_register_field(\"", ret.jsname,
				      "\", ",
...
{
  if (TREE_CODE (type) == POINTER_TYPE) {
    wasm32_jsexport_type (TREE_TYPE (type), opts, decls);
    return;
  }
  if (TYPE_CANONICAL (type) != type) {
    wasm32_jsexport_type (TYPE_CANONICAL (type), opts, decls);
    return;
  }
  if (TREE_CODE (type) == RECORD_TYPE) {
    wasm32_jsexport_record_type (type, opts, decls);
    return;
  }
}

void
wasm32_jsexport_function_decl (tree decl, struct wasm32_jsexport_opts *opts,
			      vec<struct wasm32_jsexport_decl> *decls)
{
  struct wasm32_jsexport_decl ret;
  ...
  if (TREE_CODE (decl) == FUNCTION_DECL)
      ...
      while (arg_types)
	{
	  ...
	  if (first)
	    {
	      classtypestr = wasm32_jsexport_format_type (build_pointer_type (arg_type));
	      first = false;
	    }

	  types = concat (types, wasm32_jsexport_format_type (arg_type), "; ", NULL);
	}
    }
  typestr = wasm32_jsexport_format_type (build_pointer_type (type));
  rettypestr = wasm32_jsexport_format_type (TREE_TYPE (type));
  ...
  ret.fragments.safe_push(concat (types,
				  "wasm32_register_function(\"", ret.jsname,
				  "\", ", "(", typestr, ")", NULL));
...
void
wasm32_jsexport_type_decl (tree node, struct wasm32_jsexport_opts *opts,
			  vec<struct wasm32_jsexport_decl> *decls)
{
  wasm32_jsexport_type (TREE_TYPE (node), opts, decls);
}

void
wasm32_jsexport_var_decl (tree decl, struct wasm32_jsexport_opts *opts,
			 vec<struct wasm32_jsexport_decl> *decls)
{
  struct wasm32_jsexport_decl ret;
  tree type = TREE_TYPE (decl);
  ...
  type = wasm32_canonicalize_type (type);
  const char *typestr = wasm32_jsexport_format_type (build_pointer_type (type));
  ...
  ret.fragments = vec<const char *>();
  ret.fragments.safe_push (concat ("wasm32_register_variable(\"", ret.jsname,
				   "\", ", typestr,
				   ", \"", type_jsname, "\", ", NULL));
  ret.fragments.safe_push (NULL);
...
void
wasm32_jsexport_decl (tree node, struct wasm32_jsexport_opts *opts,
		     vec<struct wasm32_jsexport_decl> *decls)
{
  switch (TREE_CODE (node))
    {
    case FUNCTION_DECL:
      wasm32_jsexport_function_decl (node, opts, decls);
      return;
    case TYPE_DECL:
      wasm32_jsexport_type_decl (node, opts, decls);
      return;
    case VAR_DECL:
      wasm32_jsexport_var_decl (node, opts, decls);
      return;
    default:
      printf("unknown code:\n");
      debug_tree (node);
...
void
wasm32_jsexport (tree node, struct wasm32_jsexport_opts *opts,
		vec<struct wasm32_jsexport_decl> *decls)
{
#if 0
  ...
  if (DECL_P (node)) {
    wasm32_jsexport_decl (node, opts, decls);
    return;
  } else if (TYPE_P (node)) {
    wasm32_jsexport_type (node, opts, decls);
    return;
  } else
    {
